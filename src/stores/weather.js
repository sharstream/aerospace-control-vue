...
export const useWeatherStore = defineStore('weather', {
  ...
  getters: {
    ...
    getHazardById: (state) => (id) => {
      return state.weatherHazards.find(hazard => hazard.id === id)
    },

    // Get hazards by type
    getHazardsByType: (state) => (type) => {
      return state.weatherHazards.filter(hazard => hazard.type === type)
    },

    // Get hazards by severity
    getHazardsBySeverity: (state) => (severity) => {
      return state.weatherHazards.filter(hazard => hazard.severity === severity)
    },
    ...
    // Get active hazards
    activeHazards: (state) => {
      return state.weatherHazards.filter(hazard => hazard.active !== false)
    }
  },
